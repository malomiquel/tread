...
export default function SendOverWifi() {
    ...
    useCallback(() => {
      ...
      const begin = async () => {
        if (!handoverAvailable()) {
          setStage({
            step: "over",
            why: "Cette version de l'app ne peut pas ouvrir de serveur. Elle demande une version installée, pas Expo Go. Le transfert par fichier, lui, marche partout.",
          });
          return;
        }
        ...
        if (everything.runs.length === 0 && everything.plan === null) {
          setStage({ step: "over", why: "Cette app n'a encore ni course ni programme à envoyer." });
          return;
        }
      ...
      begin().catch((cause: unknown) => {
        if (active) {
          setStage({
            step: "over",
            why: cause instanceof Error ? cause.message : "Le serveur n'a pas démarré.",
          });
        }
      });
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      void live.current?.stop();
      live.current = null;
      setStage({ step: "over", why: "Le temps est écoulé. Reviens sur cet écran pour recommencer." });
    }, 1000);
  ...
  if (stage.step === "packing") {
    return (
      <View style={styles.screen}>
        ...
        <Text style={styles.waiting}>Préparation de tes courses…</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.screen}>
      <Text style={styles.lede}>
        Sur le nouveau téléphone : Réglages, Transfert, Recevoir. Puis vise ce code.
      </Text>
      ...
      <Text style={styles.countdown}>
        {`Actif encore ${Math.floor(left / 60)}:${String(left % 60).padStart(2, "0")}`}
      </Text>
      <Text style={styles.note}>
        Les deux téléphones doivent être sur le même WiFi. Le partage s&apos;arrête tout seul
        en quittant cet écran, et dans tous les cas au bout de{" "}
        {Math.round(HANDOVER_TIMEOUT_MS / 60000)} minutes.
      </Text>
    </View>
  );
}
