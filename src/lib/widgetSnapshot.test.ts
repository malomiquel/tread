...
test("the week and the goal, written out for the widget", () => {
  const snapshot = widgetSnapshot({
    weekDistanceM: 18_400, weekRuns: 3, weekDurationS: 9730, goal: { kind: "distance", target: 25_000 }, next: null, now,
  });
  ...
  assert.equal(snapshot.goalText, "6,60 km pour tenir 25 km");
  assert.equal(snapshot.next, null);
});

test("a goal already met is said so, and never shows over full", () => {
  const snapshot = widgetSnapshot({
    weekDistanceM: 30_000, weekRuns: 4, weekDurationS: 10_000, goal: { kind: "distance", target: 25_000 }, next: null, now,
  });
  assert.equal(snapshot.goalShare, 1);
  assert.equal(snapshot.goalText, "Objectif de 25 km atteint");
});

test("no goal and no runs still make a widget", () => {
  const snapshot = widgetSnapshot({ weekDistanceM: 0, weekRuns: 0, weekDurationS: 0, goal: null, next: null, now });
...
test("the next session says when, what, and at what pace", () => {
  ...
  const snapshot = widgetSnapshot({
    weekDistanceM: 0, weekRuns: 0, weekDurationS: 0, goal: null, now,
  ...
  const today = widgetSnapshot({ ...{ weekDistanceM: 0, weekRuns: 0, weekDurationS: 0, goal: null, now }, next: { at: now, name: "x", kind: "y", targetSKm: 300 } });
  assert.equal(today.next?.when, "Aujourd'hui");
});

test("a time goal is said in hours", () => {
  const now = new Date(2026, 8, 23, 12).getTime();
  const snapshot = widgetSnapshot({
    weekDistanceM: 10_000, weekRuns: 2, weekDurationS: 5400, goal: { kind: "time", target: 3 * 3600 }, next: null, now,
  });
  assert.equal(snapshot.goalShare, 0.5);
  assert.equal(snapshot.goalText, "1 h 30 pour tenir 3 h");
});
